...
<%
	...
	SQL = "SELECT data FROM evento WHERE id = " & evento
	Set antigoRes = dbConnection.Execute(SQL)
	dataAntiga = antigoRes("data")
	...
	SQL = SQL & "','" & horario & "','" & autor & "','" & dataAntiga & "','" & tipo & "','" & observacoes & "','" & contacto & "','" & autores_fotos
	SQL = SQL & "','" & distrito & "','" & pais & "')"
	dbConnection.Execute(SQL)

	SQL = "SELECT id FROM evento WHERE autor = " & autor & " ORDER BY id DESC"
	Set eventoRes = dbConnection.Execute(SQL)
	...
	Menu 6, 1, "MUDAR EVENTO"
%>
	<font size="+1" color="white" face="arial"><b>Evento alterado com sucesso</b></font>
<% end if %>
